...
class BirthCertificateService {
  static async getUserData(authUserId) {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        ...
        if (allData.length > 1) {
          console.warn(`Multiple users found for auth_user_id: ${authUserId}. Using the first one.`);
        }
  ...
  static async getSpecificRequestData(authUserId, requestId) {
    ...
    try {
      ...
      if (requesterError) {
        ...
        throw new Error(`Failed to fetch specific request: ${requesterError.message}`);
      }
  ...
  static async getLatestApprovedRequesterData(userId) {
    console.log('Fetching latest approved/completed requester data for user ID:', userId);
    ...
    try {
      ...
      const requestersWithStatus = await Promise.all(
        requesterData.map(async (req) => {
          ...
          if (statusError) {
            console.error(`Error fetching status for request ${req.req_id}:`, statusError);
            return { ...req, status: null };
          }
      ...
      console.log('Requesters with status:', requestersWithStatus.map(req => ({
        req_id: req.req_id,
      ...
      if (approvedRequests.length === 0) {
        console.log('No approved/completed requester data found for user:', userId);
        console.log('Available statuses:', requestersWithStatus.map(req => req.status?.status_current).filter(Boolean));
        return null;
      }
  ...
  static async getFormData(authUserId, requestId = null) {
    try {
      if (requestId) {
        ...
        const userData = await this.getUserData(authUserId);
        const requesterData = await this.getLatestApprovedRequesterData(userData.user_id);
...
const useScriptLoader = () => {
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      const scripts = document.querySelectorAll(`script[src="${SCRIPT_URLS.HTML2CANVAS}"], script[src="${SCRIPT_URLS.JSPDF}"]`);
...
const useFormData = (authUserId) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const data = await BirthCertificateService.getFormData(authUserId, requestId);
...
const usePDFGenerator = (scriptsLoaded) => {
  const generatePDF = useCallback(async () => {
    ...
    heightLeft -= PDF_CONFIG.pageHeight;
...
const FormField = React.memo(({ label, value, style }) => (
  <div className="form-field">
    ...
    <div className="form-display-underline" style={{ 
      ...style, 
  ...
  <div className="form-container" style={{ padding: "2rem", textAlign: "center" }}>
    <div style={{
      ...
      <p style={{ marginBottom: 0 }}>
        You don't have any approved or completed birth certificate requests that allow access to this form. 
        Your request must be approved or completed before you can view and download the certificate form.
      </p>
    </div>
    <button 
      onClick={onGoBack}
...
const InvoiceViewer = () => {
  ...
  const { formData, loading: formLoading, error: formError, hasAccess, refetch } = useFormData(currentUser?.id);
  ...
  const handleDownloadImage = useCallback(async () => {
    try {
      ...
      alert("Error generating image. Please try using the Print option and save as PDF instead.");
    }
  }, [generateImage]);
  ...
  if (formError) {
    return <ErrorDisplay error={formError} onRetry={refetch} onGoBack={handleGoBack} />;
  }
  ...
  if (!formData) {
    return (
      <div className="form-container" style={{ padding: "2rem", textAlign: "center" }}>
        ...
        <button onClick={handleGoBack} style={{ marginTop: "1rem", padding: "0.5rem 1rem" }}>
          Go Back
        </button>
      </div>
    );
      ...
      <div className="form-container">
        ...
        <div className="no-print">
          ...
          <div style={{
            ...
            <strong>✅ Status:</strong> Your request has been {formData.requestStatus}. You can now download the certificate form.
          </div>
        ...
        <div className="form-content" style={blackTextStyle}>
          ...
          <div className="important-notice">
            ...
            <div className="notice-content" style={blackTextStyle}>
              ...
              <p>2. An authorization letter and ID of the document owner with the ID of the requester are required.</p>
              <p>3. Prepare Php 300.00 for the claiming of the owner's birth certificate.</p>
            </div>
          </div>
          ...
          <div className="main-form-section">
            ...
            <div className="form-group">
              ...
              <FormField label="Last name" value={formData.ownerLastName} style={blackTextStyle} />
              <FormField label="First name" value={formData.ownerFirstName} style={blackTextStyle} />
              <FormField label="Middle name" value={formData.ownerMiddleName} style={blackTextStyle} />
              <FormField label="Date of Birth" value={formData.ownerDateOfBirth} style={blackTextStyle} />
              <FormField label="Place of Birth" value={formData.placeOfBirth} style={blackTextStyle} />
            </div>

            {/* Parents Information */}
            <div className="parents-section">
              <div className="parent-group">
                <h4 className="parent-title" style={blackTextStyle}>Name of Father</h4>
                <FormField label="Last name" value={formData.fatherLastName} style={blackTextStyle} />
                <FormField label="First name" value={formData.fatherFirstName} style={blackTextStyle} />
                <FormField label="Middle name" value={formData.fatherMiddleName} style={blackTextStyle} />
              </div>

              <div className="parent-group">
                <h4 className="parent-title" style={blackTextStyle}>Name of Mother</h4>
                <FormField label="Last name" value={formData.motherLastName} style={blackTextStyle} />
                <FormField label="First name" value={formData.motherFirstName} style={blackTextStyle} />
                <FormField label="Middle name" value={formData.motherMiddleName} style={blackTextStyle} />
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <FormField label="First name" value={formData.requesterFirstName} style={blackTextStyle} />
              <FormField label="Last name" value={formData.requesterLastName} style={blackTextStyle} />
              <FormField label="Contact Number" value={formData.requesterContact} style={blackTextStyle} />
              <FormField label="Request Purpose" value={formData.requestPurpose} style={blackTextStyle} />
            </div>
          </div>

          {/* Bottom Notice */}
          <div className="bottom-notice">
            <div className="notice-title" style={blackTextStyle}>NOTICE</div>
